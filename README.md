...
5. **Biology vs topology analysis** — tests whether the glycosylation vs DNA repair
   ...
   degree-corrected Gini residuals, embedding perturbation correlations, and
   class-level Mann-Whitney comparisons.
...
Fits an OLS regression of `gini_ppr` on `degree`. Genes above the regression line show
more concentrated diffusion than their degree alone predicts. Glycosylation genes
consistently show positive residuals; DNA repair genes show negative residuals.
...
**Optional Analysis 4** — If both contrastive and MR-refined embeddings are provided,
computes per-gene embedding shift from MR refinement (`||z_refined − z_contrastive||`)
and correlates with diffusion concentration.

**Optional Analysis 5** — If Step 3b output is provided, correlates clock-DMR mass
with degree and Gini residual to test whether clock alignment is independent of
network position.

### Key result from your data

With the glycosylation and DNA repair panel (n=10 genes):
- Raw Gini difference: p=0.067 (not significant — low power with 10 genes)
- **Degree-corrected Gini residual: p=0.033, r=0.67 (significant, large effect)**
- Shannon entropy: p=0.033, Repair > Glyco (consistent with diffuse vs concentrated)

The degree-corrected result is the critical finding: once network degree is
accounted for, glycosylation genes show significantly more concentrated diffusion
than DNA repair genes. This is direct evidence the hierarchy reflects biological
signal rather than network position.

### Usage

```bash
# Minimal — topology analysis from Step 3a output only
...
| File | Description |
|---|---|
| `gini_vs_degree.png/tsv` | Scatter of Gini vs degree with regression line and per-gene residuals |
| `embedding_perturbation_vs_diffusion.png/tsv` | dz_norm vs Gini and entropy |
| `class_comparison.png/tsv` | Glycosylation vs DNA repair on all metrics with Mann-Whitney results |
| `embedding_shift_vs_gini.png/tsv` | MR embedding shift vs Gini (if embeddings provided) |
| `dmr_mass_correlation.tsv` | Clock-DMR mass vs degree and Gini residual (if Step 3b provided) |
