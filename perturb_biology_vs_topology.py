...
Four analyses are performed:
  ...
  4. Clustering coefficient analysis (requires --edge_path)
     Tests whether concentrated diffusion is explained by clustering —
     whether a gene's neighbours are also connected to each other,
     forming a tight clique. Computes per-gene clustering coefficient,
     fits a residual line (same approach as Analysis 1 but for
     clustering), and compares glycosylation vs DNA repair on
     clustering-corrected Gini residuals. This addresses the
     higher-order topology confound that degree alone does not capture.
...
--edge_path         Edge TSV file (gene-symbol edge list, columns: from/to or
                    source/target) — optional. Required for clustering coefficient
                    analysis (Analysis 4).
...
------------
  ...
  gini_vs_clustering.png/tsv     — Analysis 4: Gini vs clustering coefficient
...
# ---------------------------------------------------------------------------
# Analysis 4: Clustering coefficient analysis
# ---------------------------------------------------------------------------

def compute_clustering_coefficients(edge_path, gene_list):
    """
    Compute per-gene clustering coefficient from an edge file.

    Clustering coefficient for gene g = number of edges between g's
    neighbours / maximum possible edges between g's neighbours.

    A value of 1.0 means all neighbours are connected to each other
    (tight clique). A value of 0.0 means no neighbours are connected.

    This directly measures whether a gene sits in a dense biological
    complex (high clustering) or acts as a bridge between communities
    (low clustering).
    """
    try:
        edges = pd.read_csv(edge_path, sep="\t", dtype=str)
    except Exception as e:
        print(f"  Could not load edge file: {e}")
        return None

    # Normalise column names
    col_map = {}
    for c in edges.columns:
        if c.lower() in ("from", "source", "gene1", "node1"):
            col_map[c] = "from"
        elif c.lower() in ("to", "target", "gene2", "node2"):
            col_map[c] = "to"
    edges = edges.rename(columns=col_map)

    if "from" not in edges.columns or "to" not in edges.columns:
        print(f"  Edge file must have from/to columns. Found: {edges.columns.tolist()}")
        return None

    # Build adjacency sets — undirected
    adj = {}
    for _, row in edges.iterrows():
        a, b = str(row["from"]).strip(), str(row["to"]).strip()
        adj.setdefault(a, set()).add(b)
        adj.setdefault(b, set()).add(a)

    # Compute clustering coefficient for each gene of interest
    clustering = {}
    for gene in gene_list:
        if gene not in adj:
            clustering[gene] = np.nan
            continue
        neighbours = adj[gene]
        k = len(neighbours)
        if k < 2:
            # Clustering undefined for degree 0 or 1 — set to 0
            clustering[gene] = 0.0
            continue
        ...
        for i in range(len(neighbour_list)):
            for j in range(i + 1, len(neighbour_list)):
                if neighbour_list[j] in adj.get(neighbour_list[i], set()):
                    n_edges += 1
        max_edges = k * (k - 1) / 2
        clustering[gene] = n_edges / max_edges

    return clustering


def analysis_gini_vs_clustering(df, edge_path, out_dir):
    """
    Tests whether concentrated diffusion can be explained by clustering
    coefficient — the higher-order topology confound that degree alone
    does not capture.

    A gene sitting inside a tight clique (high clustering) could produce
    concentrated diffusion simply because PPR mass recirculates within
    the clique — regardless of the gene's biological function.

    This analysis:
      1. Computes per-gene clustering coefficient from the edge file
      2. Fits a line of Gini vs clustering (same approach as Analysis 1)
      3. Computes clustering-corrected Gini residuals
      4. Compares glycosylation vs DNA repair on those residuals
      5. Plots the scatter with gene labels and class colours
    """
    print("\n--- Analysis 4: Gini vs clustering coefficient ---")

    clustering = compute_clustering_coefficients(edge_path, df["gene"].tolist())
    if clustering is None:
        print("  Clustering analysis skipped — edge file could not be loaded.")
        return df

    df = df.copy()
    df["clustering"] = df["gene"].map(clustering)

    valid = df["clustering"].notna()
    n_valid = valid.sum()
    print(f"  Clustering coefficients computed for {n_valid}/{len(df)} genes")

    for _, row in df.iterrows():
        status = f"clustering={row['clustering']:.3f}" if pd.notna(row['clustering']) else "clustering=NaN"
        print(f"  {row['gene']:10s}  degree={int(row['degree']):3d}  "
              f"gini={row['gini_ppr']:.3f}  {status}")

    if n_valid < 3:
        print("  Not enough genes with valid clustering — skipping residual analysis.")
        return df

    x = df.loc[valid, "clustering"].values.astype(float)
    y = df.loc[valid, "gini_ppr"].values.astype(float)
    genes_valid = df.loc[valid, "gene"].values
    ...
    df.loc[valid, "clustering_residual"] = residuals
    df.loc[valid, "gini_predicted_from_clustering"] = y_fit

    print(f"\n  OLS: gini ~ clustering   slope={slope:.4f}  r={r:.3f}  p={p_r:.3f}")
    print()
    for gene, res in zip(genes_valid, residuals):
        direction = "ABOVE" if res > 0 else "below"
        print(f"  {gene:10s}  clustering_residual={res:+.3f}  {direction}")

    # Mann-Whitney on clustering residuals
    glyco_res  = df.loc[valid & (df["gene_class"] == "Glycosylation"), "clustering_residual"].values
    repair_res = df.loc[valid & (df["gene_class"] == "DNA repair"),    "clustering_residual"].values

    if len(glyco_res) >= 2 and len(repair_res) >= 2:
        stat, p_mw, effect_r = mannwhitney(glyco_res, repair_res)
        print(f"\n  Mann-Whitney clustering residual: U={stat:.0f}  p={p_mw:.3f}  r={effect_r:.3f}")
        direction_str = "Glyco > Repair" if np.median(glyco_res) > np.median(repair_res) else "Repair > Glyco"
        print(f"  Direction: {direction_str}")
        print(f"  Mean clustering residual glycosylation: {glyco_res.mean():+.3f}")
        print(f"  Mean clustering residual DNA repair:    {repair_res.mean():+.3f}")
    else:
        p_mw, effect_r = np.nan, np.nan

    # Plot
    fig, ax = plt.subplots(figsize=(7, 5))

    colours_valid = [COLOUR_GLYCO if g in GLYCOSYLATION_GENES
                     else COLOUR_REPAIR if g in DNA_REPAIR_GENES
                     else COLOUR_OTHER
                     for g in genes_valid]

    ax.scatter(x, y, c=colours_valid, s=90, zorder=3,
               edgecolors="white", linewidths=0.5)

    # Fit line
    x_line = np.linspace(x.min() - 0.02, x.max() + 0.02, 100)
    ax.plot(x_line, intercept + slope * x_line,
            color=COLOUR_FIT, lw=1.5, ls="--", zorder=2,
            label=f"OLS fit (r={r:.2f}, p={p_r:.3f})")

    # Gene labels
    for gene, xi, yi in zip(genes_valid, x, y):
    ...
    ax.legend(handles=patches + [plt.Line2D([0], [0], color=COLOUR_FIT,
                                             ls="--", lw=1.5,
                                             label=f"OLS fit (r={r:.2f})")],
              fontsize=9, frameon=False)

    mw_str = f"Clustering residual Mann-Whitney p={p_mw:.3f}" if not np.isnan(p_mw) else ""
    ax.set_xlabel("Clustering coefficient", fontsize=11)
    ax.set_ylabel("Gini coefficient (PPR concentration)", fontsize=11)
    ax.set_title(
        "Diffusion concentration vs clustering coefficient\n"
        "Genes above the line: more concentrated than clustering predicts\n"
        + mw_str,
        fontsize=9
    )
    ax.spines["top"].set_visible(False)
    ...
    plt.tight_layout()
    out_png = os.path.join(out_dir, "gini_vs_clustering.png")
    plt.savefig(out_png, dpi=180, bbox_inches="tight")
    plt.close()
    print(f"\n  Saved: {out_png}")

    out_tsv = os.path.join(out_dir, "gini_vs_clustering.tsv")
    save_cols = ["gene", "gene_class", "degree", "clustering",
                 "gini_ppr", "clustering_residual"]
...
def write_summary(df, out_dir, slope, r_degree, p_degree,
    ...
    lines += [
        ...
        "  3. The degree-corrected Gini residual controls for degree.",
        "     The clustering coefficient analysis (Analysis 4) controls for",
        "     local clique structure. Neither controls for community structure",
        "     at the mesoscale. A rewired-network null preserving both degree",
        "     and clustering would be the most rigorous test.",
        "",
        "=" * 65,
    ]
...
def main():
    ...
    parser.add_argument("--edge_path", default=None,
                        help="Edge TSV (from/to columns, gene symbols). "
                             "Required for clustering coefficient analysis (Analysis 4).")
    ...
    if args.edge_path:
        df = analysis_gini_vs_clustering(df, args.edge_path, args.out_dir)
    else:
        print("\n  Skipping clustering analysis — provide --edge_path to enable.")
        print("  This analysis tests the higher-order topology confound that")
        print("  degree alone does not capture.")
